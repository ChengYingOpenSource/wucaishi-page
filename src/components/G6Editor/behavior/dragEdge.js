export default function (G6) {
  G6.registerBehavior('dragEdge', {
    ...
    onDragStart(e) {
      ...
      if (groupId) {
        const subProcessNode = e.target.getParent().getParent().getParent().getParent()
          .get('item');
    ...
    sameNode(e) {
      return (e.target.cfg.name == 'anchor-shape') && e.target.getParent() && e.target.getParent().getParent().get('item').get('id') === this.origin.sourceNode.get('id');
    },
    ...
    dragEdgeBeforeShowAnchor(e) {
      // const sourceGroupId = this.origin.sourceNode.getModel();
      this.graph.getNodes().forEach(node => {
        if (node.getModel().type === 'start-node') { return; }
        // if (node.getModel().target === 'start-node') { return; }
        // const targetGroupId = node.getModel();
        let flag = false;
        this.graph.getEdges().forEach(line => {
          if (line.getModel().target == node.getModel().id) {
            flag = true;
          }
        });
        if (flag) { return; }
